...
class ServiceRequestServiceTest1 {
    ...
    void getAllServiceRequests_Success() {
        ...
        assertEquals(1, serviceRequestService.getAllServiceRequests().size());
    }
    ...
    void getServiceRequestById_Success() {
        ...
        assertNotNull(serviceRequestService.getServiceRequestById(1L));
    }
    ...
    void assignManager_Success() {
        ...
        when(serviceRequestRepository.findById(1L)).thenReturn(Optional.of(serviceRequest));
        when(serviceRequestRepository. save(any())).thenReturn(serviceRequest);
        when(vehicleServiceClient. getVehicleById(anyLong())).thenReturn(vehicleResponse);
        when(vehicleServiceClient.updateVehicleStatus(anyString(), anyString())).thenReturn("OK");

        assertNotNull(serviceRequestService.assignManager(1L, request));
    }
    ...
    void assignTechnician_Success() {
        ...
        when(serviceRequestRepository.findById(1L)).thenReturn(Optional.of(serviceRequest));
        when(userServiceClient.getTechnicianById(60L)).thenReturn(new TechnicianResponse());
        ...
        assertNotNull(serviceRequestService.assignTechnician(1L, request));
    }
    ...
    void updateStatus_Success() {
        ...
        assertNotNull(serviceRequestService.updateStatus(1L, "MANAGER_ASSIGNED"));
    }
    ...
    void updateRemarks_Success() {
        ...
        assertNotNull(serviceRequestService.updateRemarks(1L, "Updated remarks"));
    }
    ...
    void payBill_Success() {
        ...
        when(serviceBillRepository.save(any())).thenReturn(bill);

        assertTrue(serviceRequestService.payBill(1L));
    }
}