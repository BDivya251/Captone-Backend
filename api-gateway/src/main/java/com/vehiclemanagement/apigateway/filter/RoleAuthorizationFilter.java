...
public class RoleAuthorizationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        boolean hasAccess = checkAccess(path, method, role, customerIdStr, technicianIdStr);
    ...
    private boolean checkAccess(String path, HttpMethod method, String role, String customerIdStr, String technicianIdStr) {
        ...
        if ("TECHNICIAN".equals(role)) {
            ...
            if (path.matches("/service-management-service/vehicle/service-requests/technician/" + technicianId + "/.*")) {
                return true;
            }
            
            if (path.matches("/service-management-service/vehicle/service-requests/\\d+/(status|remarks|inventory-usage|images)")) {
                return true;
            }
            
            if (path.matches("/service-management-service/vehicle/service-requests/(\\d+|images/\\d+)") && method == HttpMethod.GET) {
                return true;
            }
          ...
          if ("CUSTOMER". equals(role)) {
            ...
            if (path. matches("/service-management-service/vehicle/service-requests(/customer/" + customerId + "|/\\d+)?") && method == HttpMethod.GET) {
                return true;
            }
            ...
            // View images
            if (path.startsWith("/service-management-service/vehicle/service-requests/images/") && method == HttpMethod.GET) {
                return true;
            }
